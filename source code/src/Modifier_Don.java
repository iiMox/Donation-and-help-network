...
public class Modifier_Don extends JFrame {
	...
	public Modifier_Don(int code) {
		...
		String [] itemsCat = { "Maison & fournitures", "Elctroménager", "Informatique", "Elctronique", "Vetements & accessoires" };
		...
		try {
			...
			if(oc.rs.next()) {
				...
				String cat = oc.rs.getString("categorie");
				
				for(int j=0;j<comboBox.getItemCount();j++) {
					if(comboBox.getItemAt(j).equals(cat)) {
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				Don don = new Don();
				don.setCode(code);
				don.setTitle(textField.getText());
				don.setDescription(textArea.getText());
				don.setCategorie((String)comboBox.getSelectedItem());
				don.setEtat("en attend");
				don.setWilaya((String)comboBox_1.getSelectedItem());
				don.modifier_don();
			}
		});
